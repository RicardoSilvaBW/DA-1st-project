...
class CSVParser {
    public:
    CSVParser();//I created this function to read and trigger the parsing.
    bool parseFile(const std::string& filename);//Bool flag to check if the file parsing was successful, it returns false if it has insconsistent values, the project descrption implies we do this
